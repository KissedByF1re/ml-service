...
- **ML-библиотеки:** `scikit-learn` (для моделей логистической регрессии, SVM и случайного леса) и `CatBoost` (для модели градиентного бустинга). Предполагается, что модели предварительно обучены на датасете оттока клиентов (например, на открытых данных из Kaggle) и сохранены для последующего использования в сервисе.
...
- **Прочее:** Используются инструменты обеспечения качества кода, такие как pre-commit hooks (настройки в репозитории), а зависимости проекта управляются через файл pyproject.toml (например, с помощью Poetry или аналогичного пакетного менеджера).
...
- **Запуск предсказания churn**
  - Принимает данные клиента и `service_order_id`
...
- `Service` – описание доступных ML-сервисов (LR, SVM, RandomForest, CatBoost), включая:
  - Название
  - Код модели
...
- `Transaction` – учёт операций:
  - Типы: `credit` (зачисление) и `debit` (списание)
...
- `ServiceOrder` – заказ на предсказание:
  ...
  - Флаг `is_provided` – завершён ли заказ
...
- **Celery-задача `predict_task`**
  - Загружает модель
  - Применяет к входным данным (в сериализованном виде или из файла)
  - Возвращает результаты (файл или JSON)

- **Интеграция с Celery**
  - Очередь: RabbitMQ
  - Хранение состояния задач: Redis
...
- `api` — Django-приложение (с Gunicorn или Uvicorn)
- `db` — PostgreSQL (БД)
...
│       ├── Dockerfile        # Образ для контейнера веб-приложения
│       ├── pyproject.toml    # Зависимости проекта (настройки Poetry)
│       ├── static/           # Статические файлы (например, для админ-панели Django)
│       └── src/
│           └── api/
...
│               │       └── views.py         # Вью для приема запроса на прогноз (запуск задачи)
...
│               │   └── gunicorn.conf.py     # (Опционально) конфиг Gunicorn
...
# Дальнейшие манипуляции и настройку вы можете выполнять непосредственно в IDE
```
